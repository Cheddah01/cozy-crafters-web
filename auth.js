...
function ccAuthSafeImage(value) {
  return typeof ccSafeImageUrl === 'function' ? ccSafeImageUrl(value) : String(value ?? '');
}
...
function ccRenderAuthNav() {
  ...
  if (user) {
    const username = ccAuthEscape(user.username || 'Player');
    const avatar = ccAuthEscape(ccAuthSafeImage(user.avatar));
    const adminBtn = user.role === 'admin'
      ? `<a href="admin.html" style="font-family:'Fredoka',sans-serif; font-size:0.75rem; font-weight:600; background:rgba(232,155,107,0.18); border:1px solid rgba(232,155,107,0.4); color:var(--sun); padding:0.3rem 0.7rem; border-radius:8px; text-decoration:none; transition:all 0.2s;" onmouseover="this.style.background='rgba(232,155,107,0.3)'" onmouseout="this.style.background='rgba(232,155,107,0.18)'">⚙ Admin</a>`
      : '';
    authEl.innerHTML = `
      <img src="${avatar}" alt="" style="width:30px; height:30px; border-radius:50%; border:2px solid rgba(232,155,107,0.4);" />
      <span style="font-family:'Fredoka',sans-serif; font-size:0.88rem; font-weight:600; color:var(--cream);">${username}</span>
      ${adminBtn}
      <button onclick="ccLogout()" style="font-family:'Fredoka',sans-serif; font-size:0.75rem; font-weight:600; background:none; border:1px solid currentColor; color:var(--cream); opacity:0.55; padding:0.3rem 0.6rem; border-radius:8px; cursor:pointer; transition:opacity 0.2s;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.55'">Log out</button>
    `;
  } else {
    authEl.innerHTML = `
  ...
  if (mobileMenu) {
    ...
    if (user) {
      ...
      const mAdminBtn = user.role === 'admin'
        ? `<a href="admin.html" style="font-family:'Fredoka',sans-serif; font-size:0.9rem; font-weight:600; background:rgba(232,155,107,0.18); border:1px solid rgba(232,155,107,0.4); color:var(--sun); padding:0.5rem 1.2rem; border-radius:10px; text-decoration:none; display:inline-block;">⚙ Admin Panel</a>`
        : '';
      mobileAuth.innerHTML = `
        <div style="display:flex; flex-direction:column; align-items:center; gap:0.8rem;">
          <img src="${avatar}" alt="" style="width:48px; height:48px; border-radius:50%; border:2px solid rgba(232,155,107,0.4);" />
          <span style="font-family:'Fredoka',sans-serif; font-size:1rem; font-weight:600; color:var(--cream);">${username}</span>
          ${mAdminBtn}
          <button onclick="ccLogout()" style="font-family:'Fredoka',sans-serif; font-size:0.85rem; font-weight:600; background:none; border:1px solid currentColor; color:var(--cream); opacity:0.7; padding:0.4rem 1rem; border-radius:8px; cursor:pointer;">Log out</button>
        </div>`;
    } else {
      mobileAuth.innerHTML = `
