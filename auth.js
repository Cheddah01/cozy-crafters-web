  ...
  if (mobileMenu) {
    ...
    if (user) {
      ...
      const mAdminBtn = user.role === 'admin'
        ? `<a class="cc-auth-admin" href="admin.html" style="font-family:'Fredoka',sans-serif; font-size:0.9rem; font-weight:600; background:rgba(244,201,93,0.15); border:1px solid rgba(244,201,93,0.3); color:#F4C95D; padding:0.5rem 1.2rem; border-radius:10px; text-decoration:none; display:inline-block;">⚙ Admin Panel</a>`
        : '';
      mobileAuth.innerHTML = `
        <div style="display:flex; flex-direction:column; align-items:center; gap:0.8rem;">
          <img class="cc-auth-avatar" src="${avatar}" alt="" style="width:48px; height:48px; border-radius:50%; border:2px solid rgba(244,201,93,0.3);" />
          <span class="cc-auth-name" style="font-family:'Fredoka',sans-serif; font-size:1rem; font-weight:600; color:#FFF4DC;">${username}</span>
          ${mAdminBtn}
          <button class="cc-auth-logout" onclick="ccLogout()" style="font-family:'Fredoka',sans-serif; font-size:0.85rem; font-weight:600; background:none; border:1px solid rgba(255,244,220,0.2); color:#FFF4DC; padding:0.4rem 1rem; border-radius:8px; cursor:pointer;">Log out</button>
        </div>`;
    } else {
      mobileAuth.innerHTML = `
