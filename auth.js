...
function ccRenderAuthNav() {
  ...
  if (user) {
    ...
    const adminBtn = user.role === 'admin'
      ...
      : '';
    authEl.innerHTML = `
      <img src="${avatar}" alt="" style="width:30px; height:30px; border-radius:50%; border:2px solid rgba(244,201,93,0.3);" />
      <span style="font-family:'Fredoka',sans-serif; font-size:0.88rem; font-weight:600; color:#FFF4DC;">${username}</span>
      ${adminBtn}
    ...
    if (user) {
      ...
      mobileAuth.innerHTML = `
        <div style="display:flex; flex-direction:column; align-items:center; gap:0.8rem;">
          <img src="${avatar}" alt="" style="width:48px; height:48px; border-radius:50%; border:2px solid rgba(244,201,93,0.3);" />
          <span style="font-family:'Fredoka',sans-serif; font-size:1rem; font-weight:600; color:#FFF4DC;">${username}</span>
          ${mAdminBtn}
